...
## Install
1. Copy this `bgrid-esp32` folder into your Arduino `libraries` directory (you can rename it to `BGrid` if you prefer).
...
1. Generate a header `BGridWords.h` from the JSON files in `src/bip39-wordlist` (provided in this repo). You can run:
```bash
...
## Tests (desktop)
Compile the lightweight C++ test (no Arduino required):
```bash
g++ -std=c++17 -DARDUINO -I bgrid-esp32/src bgrid-esp32/tests/bgrid_test.cpp bgrid-esp32/src/BGrid.cpp -o /tmp/bgrid_test
/tmp/bgrid_test
```
